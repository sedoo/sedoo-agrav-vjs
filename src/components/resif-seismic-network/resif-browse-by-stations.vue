...
<script>
import router from "../../router-stations";
export default {
  name: "resif-browse-by-stations",
   router: router,
  components: {
  ...
  methods: {
    ...
    if(e.target && e.target.id== 'station-bc-level1'){
      self.goToLevel1();
     }

    if(e.target && e.target.id== 'station-bc-level2'){
      self.goToLevel2();
     }
    });
...
computeExternalBreadcrumb() {
    ...
    if (this.type == 'external') {
      ...
      if (this.isLevel1) {
        ...
        breadcrumb.innerHTML = "<span id='station-bc-level1' style='cursor:pointer'>"+this.level1Label+"</span>  > "+this.currentNetworkLabel;
      } else {
        breadcrumb.innerHTML = "<span id='station-bc-level1' style='cursor:pointer'>"+this.level1Label+"</span>  > <span id='station-bc-level2' style='cursor:pointer'>"+this.currentNetworkLabel+ "</span> > "+this.currentStationLabel
      }
    }
