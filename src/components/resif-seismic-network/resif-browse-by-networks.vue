...
<script>
import router from "../../router-networks";
export default {
  name: "resif-browse-by-networks",
   router: router,
  components: {
  ...
  methods: {
    ...
    if(e.target && e.target.id== 'network-bc-level1'){
      self.goToLevel1();
     }

    if(e.target && e.target.id== 'network-bc-level2'){
      self.goToLevel2();
     }
    });
...
computeExternalBreadcrumb() {
    ...
    if (this.type == 'external') {
      ...
      if (this.isLevel1) {
        ...
        breadcrumb.innerHTML = "<span id='network-bc-level1' style='cursor:pointer'>"+this.level1Label+"</span>  > "+this.currentNetworkLabel;
      } else {
        breadcrumb.innerHTML = "<span id='network-bc-level1' style='cursor:pointer'>"+this.level1Label+"</span>  > <span id='network-bc-level2' style='cursor:pointer'>"+this.currentNetworkLabel+ "</span> > "+this.currentStationLabel
      }
    }
